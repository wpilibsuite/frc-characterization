...
class ProgramState:
    ...
    def __init__(self):
        ...
        self.pulley_diam = DoubleVar(mainGUI)
        ...
        self.kg = DoubleVar(mainGUI)
        self.kfr = DoubleVar(mainGUI)
...
def configure_gui():
    ...
    def runAnalysis():
        ...
        STATE.kg.set('%s' % float('%.3g' % kg))
        STATE.kfr.set('%s' % float('%.3g' % kfr))
...
def _plotVoltageDomain(direction, qu, step):
    ...
    plt.scatter(
        qu[PREPARED_V_COL] - kg - kfr * np.sign(qu[PREPARED_VEL_COL]) - ka *
        qu[PREPARED_ACC_COL],
        qu[PREPARED_VEL_COL],
        marker=".",
        c="#000000",
    ...
    plt.scatter(
        step[PREPARED_V_COL] - kg - kfr * np.sign(step[PREPARED_VEL_COL]) - kv *
        step[PREPARED_VEL_COL],
...
def main():
    ...
    mainGUI.title("RobotPy Elevator Characterization Tool")

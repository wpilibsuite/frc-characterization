...
def prepare_data(data, window):
    ...
    direc = np.sign(data[ENCODER_V_COL])
...
def analyze_data(data, window=WINDOW, threshold = MOTION_THRESHOLD):
    ...
    # Concatenate forward and backward into one data set

    quasi = np.concatenate((sf, sb), axis = 1)
    step = np.concatenate((ff, fb), axis = 1)
    ...
    def _print(n, pfx, qu, step):
        ...
        direc = np.concatenate((qu[PREPARED_DIR_COL], step[PREPARED_DIR_COL]))
        volts = np.concatenate((qu[PREPARED_V_COL], step[PREPARED_V_COL]))
        time = np.concatenate((qu[PREPARED_TM_COL], step[PREPARED_TM_COL]))

        fit = _ols(vel, accel, direc, volts)
        kg, kv, ka, kfr = fit.params
        rsquare = fit.rsquared

        txt = "%s:  kv=% .4f ka=% .4f kfr=% .4f kg=% .4f r-squared=% .4f" % (
            pfx,
            kv,
            ka,
            kfr,
        ...
        plt.scatter(
            qu[PREPARED_V_COL] - kg - ka * qu[PREPARED_ACC_COL] - kfr * qu[PREPARED_DIR_COL],
            qu[PREPARED_VEL_COL],
            marker=".",
            c="#000000",
        )
        ...
        plt.scatter(
            step[PREPARED_V_COL] - kg - kv * step[PREPARED_VEL_COL] - kfr * step[PREPARED_DIR_COL],
        ...
        ax.scatter(vel, accel, volts - kfr * direc)
